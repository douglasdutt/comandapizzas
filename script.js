function main(){
  ...
  if(temitem == 1){
    ...
    alert("Pelo menos um item para continuar!");
    temitem = 0;
  }
}
...
function gerainterfacecomanda(valor){
  ...
  pizzas = document.getElementsByClassName('pizzas');
  adicionais = document.getElementsByClassName('adicionais');
  pasteis = document.getElementsByClassName('pasteis');
  ...
  if(pizzas.length == 1){pizzas[0].remove();}
  if(adicionais.length == 1){adicionais[0].remove();}
  if(pasteis.length == 1){pasteis[0].remove();}
